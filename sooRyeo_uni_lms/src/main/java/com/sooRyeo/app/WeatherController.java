...
public class WeatherController {
	...
	// 현재 시간 날씨 하이차트
	@ResponseBody
	@PostMapping(value="/weatherXMLtoJSON.lms", produces="text/plain;charset=UTF-8")
	public String weatherXMLtoJSON(HttpServletRequest request) {
		
		String str_jsonObjArr = request.getParameter("str_jsonObjArr");
		// System.out.println("str_jsonObjArr 확인 : " + str_jsonObjArr);
		...
		for(String jsonObj : arr_str_jsonObjArr) {
			...
			for(int i=0; i<locationArr.length; i++) {
				if(jsonObj.indexOf(locationArr[i]) >= 0 && jsonObj.indexOf("북") == -1 && jsonObj.indexOf("서청주") == -1) {
					// 북춘천,춘천,북강릉,강릉,북창원,창원,북부산 등이 있으므로  "북" 이 있는 것은 제외하도록 한다. 또한 서청주(예)도 제외하도록 한다.
					result += jsonObj+",";
					break;
				}		
			}// end of for-----------------
