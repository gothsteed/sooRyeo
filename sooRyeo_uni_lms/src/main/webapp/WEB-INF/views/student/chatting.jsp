 	...
 	websocket.onmessage = function(event){
		...
		if (message.messageType === "CHAT") {
			if (message.senderId == senderId && message.senderType == senderType) {
				$("div#chatMessage").append("<div style='background-color: #ffff80; display: inline-block; max-width: 60%; float: right; padding: 7px; border-radius: 15%; word-break: break-all;'>" + message.content + "</div> <div style='display: inline-block; float: right; padding: 20px 5px 0 0; font-size: 7pt;'>"+currentTime+"</div> <div style='clear: both;'>&nbsp;</div>");
			} else {
				$("div#chatMessage").append("<div style='background-color: #e0e0e0; display: inline-block; max-width: 60%; float: left; padding: 7px; border-radius: 15%; word-break: break-all;'>" + message.content + "</div> <div style='display: inline-block; float: left; padding: 20px 5px 0 0; font-size: 7pt;'>"+currentTime+"</div> <div style='clear: both;'>&nbsp;</div>");
			}
		} else if (message.messageType === "ALERT") {
			$("div#chatMessage").append("<div style='text-align: center; color: red;'>" + message.content + "</div>");
		}
